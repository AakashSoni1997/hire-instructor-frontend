...
const StepTwoSubscription = ({ amount, setActiveStep }) => {
    ...
    const handleNextSubmitButton = () => {
        if (Object.keys(state).length > 0) {
            navigate(`/subscription/checkout/${id}`, { state: { ...prevPageData, ...state } });
    ...
    const handlePrevious = () => {
        navigate(`/subscription/selectplan/${id}`, { state: prevPageData });
        setActiveStep('selectplan');
    }
...
export default StepTwoSubscription;